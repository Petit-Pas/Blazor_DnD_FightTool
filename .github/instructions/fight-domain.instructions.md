...
- **Events**: Use standard `EventHandler<T>` pattern (e.g., `OnFighterRemoved`, `OnFighterUpdated`). `OnFighterUpdated` is fired via `NotifyFighterUpdated(Guid)` when a fighter's state is mutated in-place (same object reference) — this is needed because Blazor won't detect changes on the same reference.
